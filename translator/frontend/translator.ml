...
and trans_lib_type : string -> typ  (* TODO: Need to check its generality *)
= fun str ->
...
= fun json ->
  ...
  | `String "Identifier" ->
     (try
       ...
       let vinfo = (* Note: the information is not exact at the moment, but will be updated in the preprocessing phase. *)
         { vloc = loc;
           is_gvar = false;
     ...
     (match value_of "kind" json with
      ...
      | `String "string" ->
         let s = 
           ...
           with _ -> json |> value_of "hexValue" |> to_string in (* checkout real-bench/029.sol: hex"..." *)
         Str s
  ...
  | `String "Conditional" -> (* Note: we assume only expressions exist in each branch. *)
  ...
  | `List l ->
     let reversed_args =
       List.fold_left (fun acc j ->
         ...
         (* try (trans_expression j)::acc
         with | _ -> raise (Failure "Unsupported: trans_functionCallArguments") (* Note: NameValueList is unsupported *) *)
       ) [] l
     in 
     List.rev (reversed_args)
  | `Null -> [] (* Note: If there is no arguments, `Null is returend, not `List []. *)
  | _ -> assert false
...
= fun json ->
  ...
  | `String "BinaryOperation" -> Skip (*1227 line at 300.sol - does nothing*)
...
= fun json ->
  ...
  | `String "VariableDeclarationStatement" -> (* declaration := initialValue *)
     let decl = json |> value_of "declarations" |> to_list in (* TODO: trim here *)
     ...
     (match value_of "initialValue" json with
      | `Null -> Decl lv (* TODO: maybe desugared later *)
      | exp -> Assign (lv, trans_expression exp, loc)) (* TODO: Do we need Decl aside?, e.g., for array length. *)
  ...
  List.fold_left (fun acc j ->
    ...
    (match node_typ with 
     | `String "ContractDefinition" ->
      (* Note: currently, assume "contractKind" is "contract". Maybe it should be fixed later. *)
