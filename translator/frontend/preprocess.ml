...
= fun exp ->
  ...
  | CallTemp (Lv (MemberAccess (Cast (t,e),id,id_info,typ)), params, einfo) -> (* ... := cast(y).f(33), e.g., line 248 of 149.sol *)
  ...
  | CallTemp (e,params,einfo) ->
    if is_tuple einfo.etyp then (* Note: we currently assume function variable has the return type. *)
...
= fun stmt ->
  ...
  | Call (lvop,e,params,loc,site) when has_gas_value_modifiers e -> (* TODO: preserve gas and value information if necessary *)
  ...
  | Return (Some e,loc_ret) ->
    ...
    (match e',new_stmt with
     | Lv (Tuple ([],_)), Call (Some (Tuple ([],_)),e,args,loc,site) -> (* return f(), where f() returns void. *)
       Seq (Call (None,e,args,loc,site), Return (None,loc_ret)) (* e.g., line 151 of 117.sol *)
...
= fun lib stmt ->
  ...
  | Call (lvop,Lv (Var (v,vinfo)),args,loc,site) ->
     ...
     * Also, libraries cannot be inheritied from, i.e., libaries cannot be parents.
     * Thus, it is okay to simply add a library name for all function calls. *)
...
= fun p ->
  ...
  List.fold_left (fun acc c ->
     ...
     * that means the super keywords in the contract need not be changed,
     * because the contract is irrelevant with the main contract,
     * i.e., the contract is dead code where there are no functions to be copied to the main contract.
     * Therefore, we skip those contracts without modifications, i.e., 'acc' in the 'else' case.
     * Without checking this condition, we may encounter exceptions,
     * since we cannot find next contracts, see 'find_next_contracts'. *) 
...
(* TODO: currently, generate getters only for uint-kind and address type variables *)
...
(* Therefore, implemented a customized fold_left2 function. *)
