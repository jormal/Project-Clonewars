...
let trans_node = Node.Node 0 (* transitional node *)
...
and lv =
  ...
  | IndexAccess of exp * exp option * typ (* exp[exp?] *) (* not sure why 'option' exists *)
...
and cinfo = {
  ...
  lib_typ_lst : (id * typ) list; (* a list of pairs of (lib name, aliased type). Note that orders do not matter. *)
  ckind : string
}
...
= fun lv loc ->
  ...
  (* NOTE: This is a must. Without this, interval analysis may produce bot results for some vars. *)
...
= fun e1 e2 ->
  (* let _ = assert (is_usual_mapping (get_type_exp e1)) in *) (* TODO *)
...
= fun t1 t2 ->
  if t1=t2 then t1
   ...
   (match t1,t2 with
    ...
    | EType (SInt n1), EType (UInt n2) when n1>n2 -> t1 (* uint can be converted into int *)
    | EType (SInt n1), EType (UInt n2) when n1<=n2 -> raise (Failure "preceding_typ : intX1 and uintX2 are not convertible if X1<=X2")
    | EType (UInt n1), EType (SInt n2) when n1<n2 -> t2 (* uint can be converted into int *)
    ...
    | Array (t1,None), Array (t2, Some n) when t1=t2 -> (* e.g., at line 117 of real-bench/367.sol *) 
      Array (t1,None)
    | Contract id1, Contract id2 -> (* e.g., at line 941 of real-bench/071.sol *)
      t2 (* Note: this is obviously incorrect. *)
