...
= fun callee gvars cnames node g ->
  ...
  if is_exit node then g
    ...
    replace_node node (copied_node, copied_stmt) g

(* returns (entry_node, exit_node, whole graph) *)
...
= fun gvars cnames fmap caller n g ->
  ...
  (*| Call (lvop,e,args,loc,site)
    when !Options.exploit && is_recursive_callnode ctx site n ->
    (true, ctx, add_node_stmt n Throw g) *)
      ...
      let output_stmt = 
        (match lvop with
         ...
         | Some lv -> (* lv <- ret_params when 'lv:= call()' *)
           ...
           Assign (lv,e,loc))
      in
      let g = add_node_stmt output_node output_stmt g in
      let g = add_edge callee_x output_node g in
      let g = List.fold_left (fun acc s -> add_edge output_node s acc) g succs in
      (true, g)
